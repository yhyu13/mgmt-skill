...
> 蒸馏管理方法论 → 知识库 + 可交互工具 + 角色化顾问，开源到 GitHub

## 这是什么

mgmt-skill 把世界顶级的管理方法论蒸馏成三类产物，让 AI 和人都能调用：

1. **方法论知识库**（MGMT-SKILL.md）——结构化的管理方法论条目，可检索
2. **可交互工具**（HTML原型）——把方法论做成工具，不是读而是用
3. **角色化顾问**（Advisor Skill）——思维模型 + mgmt方法论 + 角色模拟 = 可对话的企业家顾问

**三层产物的关系**：方法论知识库是"条目可检索"，可交互工具是"方法论可操作"，角色化顾问是"人物可对话"。思维模型基于公开资料提炼，方法论来自mgmt知识库。
...
├── advisor-skills/           # 🆕 角色化顾问（CC BY-NC 4.0）
│   ├── renzhengfei-advisor.md   # 任正非管理顾问（灰度思维+华为方法论）
│   ├── zhangyiming-advisor.md   # 张一鸣管理顾问（延迟满足+字节方法论）
│   └── bezos-advisor.md         # 贝佐斯管理顾问（Day1+亚马逊方法论）
├── distill/                  # 蒸馏流程和模板（MIT）
│   ├── MGMT-SKILL-模板.md
│   └── 蒸馏流程.md
├── tools/                    # 可交互工具
...
## 角色化顾问（Advisor Skills）

角色化蒸馏产物：思维模型（基于公开资料提炼）+ mgmt方法论 + 角色模拟 = 可对话的企业家顾问。

| 顾问 | 思维模型 | 方法论武装 | 状态 |
|------|---------|-----------|------|
| 任正非 | 灰度思维/自我批判/危机意识 | 华为13个方法论（BEM/BLM/IPD/PBC等） | ✅ v1.0 |
| 张一鸣 | 延迟满足/算法思维/CNC | 字节12个方法论（OKR/AB测试/CNC等） | ✅ v1.0 |
| 贝佐斯 | 长期主义/Day1/逆向工作法 | 亚马逊10个方法论（领导力原则/六页纸等） | ✅ v1.0 |

**使用方式**：把角色卡作为AI的system prompt或skill文件加载，用户提问时AI以该企业家的思维方式和语气回答，同时调用对应方法论给出落地建议。

**后续计划**：逐步补齐14个重叠人物→组织级角色卡→学科级角色卡。详见 [docs/角色化蒸馏执行计划.md](docs/角色化蒸馏执行计划.md)。
